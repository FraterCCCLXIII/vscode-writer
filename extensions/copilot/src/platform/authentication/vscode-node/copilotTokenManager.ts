...
import { STANDALONE_THIRD_PARTY_CHAT_TOKEN_STUB, STANDALONE_THIRD_PARTY_CHAT_TOKEN_USERNAME, isStandaloneThirdPartyChatFromProduct } from '../common/standaloneThirdPartyChatProduct';
...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		if (isStandaloneThirdPartyChatFromProduct()) {
			this._logService.info('Copilot token: standalone third-party chat (product.json) — skipping GitHub session; using local stub token.');
			const stub = createTestExtendedTokenInfo({
				...
				token: STANDALONE_THIRD_PARTY_CHAT_TOKEN_STUB,
				username: STANDALONE_THIRD_PARTY_CHAT_TOKEN_USERNAME,
