...
export class DefaultOpenAIAgentPrompt extends PromptElement<DefaultAgentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			{tools[ToolName.CreateFile] && tools[ToolName.EditFile] && <>Never use {ToolName.CreateFile} for a file that already appears in the provided context (active file, attached files, or referenced files) — use {ToolName.EditFile} instead.<br /></>}
			{tools[ToolName.CreateFile] && <>When a user asks to "create a page" or "create a document" (e.g. "create a characters page", "create a story outline"), always populate the new file with substantive, relevant content that fulfils the request — never create an empty or placeholder file.<br /></>}
				{tools[ToolName.CoreRunInTerminal] && <>NEVER use terminal commands (echo, sed, awk, tee, cat redirection, or any other shell construct) to write or modify file contents — doing so can silently overwrite or corrupt the entire file. Always use the dedicated file-edit tools ({tools[ToolName.ReplaceString] ? ToolName.ReplaceString : ''}{tools[ToolName.MultiReplaceString] ? `/${ToolName.MultiReplaceString}` : ''}{tools[ToolName.EditFile] ? `/${ToolName.EditFile}` : ''}) to make changes. If an edit tool fails, report the error to the user instead of falling back to terminal commands.<br /></>}
