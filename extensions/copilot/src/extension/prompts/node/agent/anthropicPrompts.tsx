	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='toolUseInstructions'>
				...
				{tools[ToolName.CoreRunInTerminal] && <>NEVER use terminal commands (echo, sed, awk, tee, cat redirection, or any other shell construct) to write or modify file contents — doing so can silently overwrite or corrupt the entire file. Always use the dedicated file-edit tools to make changes. If an edit tool fails, report the error to the user instead of falling back to terminal commands.<br /></>}
	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='toolUseInstructions'>
				...
				{tools[ToolName.CoreRunInTerminal] && <>Don't call the {ToolName.CoreRunInTerminal} tool multiple times in parallel. Instead, run one command and wait for the output before running the next command.<br /></>}
				{tools[ToolName.ExecutionSubagent] && <>Don't call {ToolName.ExecutionSubagent} multiple times in parallel. Instead, invoke one subagent and wait for its response before running the next command.<br /></>}
				{tools[ToolName.CreateFile] && <>When creating files, be intentional and avoid calling the {ToolName.CreateFile} tool unnecessarily. Only create files that are essential to completing the user's request. Never use {ToolName.CreateFile} for a file that already appears in the provided context (active file, attached files, or referenced files) — use an edit tool such as {tools[ToolName.EditFile] ? ToolName.EditFile : 'edit_file'} instead. When a user asks to "create a page" or "create a document" (e.g. "create a characters page", "create a story outline"), always populate the new file with substantive, relevant content that fulfils the request — never create an empty or placeholder file.<br /></>}
				When invoking a tool that takes a file path, always use the absolute file path shown in the context. If the file has a scheme like untitled: or vscode-userdata:, then use a URI with the scheme.<br />
				{tools[ToolName.CoreRunInTerminal] && <>NEVER use terminal commands (echo, sed, awk, tee, cat redirection, or any other shell construct) to write or modify file contents — doing so can silently overwrite or corrupt the entire file. Always use the dedicated file-edit tools to make changes. If an edit tool fails, report the error to the user instead of falling back to terminal commands.<br /></>}
				{!tools.hasSomeEditTool && <>You don't currently have any tools available for editing files. If the user asks you to edit a file, you can ask the user to enable editing tools or print a codeblock with the suggested changes.<br /></>}
				{!tools[ToolName.CoreRunInTerminal] && <>You don't currently have any tools available for running terminal commands. If the user asks you to run a terminal command, you can ask the user to enable terminal tools or print a codeblock with the suggested command.<br /></>}
